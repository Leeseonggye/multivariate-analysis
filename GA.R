...
Ploan <- read.csv("C:/Users/이성계/Desktop/수업/multivariate-data-analysis-master/multivariate-data-analysis-master/08 Logistic Regression/Personal Loan.csv")
...
full_model <- glm(Ploan_target ~ ., family=binomial, Ploan_trn) #family=binomial(logistic 조건)
summary(full_model)
...
forward_model <- step(glm(Ploan_target ~ 1, data = Ploan_trn), #상수항만 있는 항을 만들어라 
                      scope = list(upper = as.formula(tmp_xy), lower = Ploan_target ~ 1),#탐색하고 싶은 범위가 어디 
...
fit_F1 <- function(string){  #입력으로 받는 string은 하나의 염색체다
  sel_var_idx <- which(string == 1)
  # Use variables whose gene value is 1
  sel_x <- x[, sel_var_idx] #x is whole input data
  xy <- data.frame(sel_x, y)
  # Training the model
  GA_lr <- glm(y ~ ., family = binomial, data = xy)
  GA_lr_prob <- predict(GA_lr, type = "response", newdata = xy) #type=response -> probability!
  ...
  return(GA_perf[6]) #F-1 measure 기준으로 가장 좋은것을 찾아라.
} # input: chromosome, output: f-l measure
...
GA_F1 <- ga(type = "binary", fitness = fit_F1, nBits = ncol(x), #nbits number ofchromosome variable (columns)
            names = colnames(x), popSize = 50, pcrossover = 0.5, #서로다른 chromosome을 50개씩 들고간다. pcrossover=0.5 반반씩 섞는다
            pmutation = 0.01, maxiter = 100, elitism = 2, seed = 123) #eltism=2 부모 세대 중에서 1,2등은 그대로 살아남는다
end_time <- proc.time()
end_time - start_time
...
Ridge_model <- glmnet(Ploan_trn_X, Ploan_trn_y, family = "binomial", alpha = 0) #alpha=0 ridge
